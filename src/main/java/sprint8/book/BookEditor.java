 ...
 * В BookEditor хранятся специальные обработчики — за них отвечают объекты интерфейсов HeaderDecorator
 * для форматирования заголовка и LineProcessor для строк.
 * Мы определили реализацию каждого из обработчиков в обычных Java-классах — ToUpperCaseHeaderDecorator
 * приводит название книги к верхнему регистру, а CapitalizeFirstLetterProcessor делает первую букву в каждой строке
 * заглавной.
 * Ваша задача — переписать код с использованием лямбда-функций вместо классов ToUpperCaseHeaderDecorator и
 * CapitalizeFirstLetterProcessor, а также добавить новый обработчик строк, который будет добавлять в конец каждой
 * строки символ переноса строки \n.
 */
