 ...
 * Теперь можно поработать с конкретным API. Сервис https://ipwhois.io/ позволяет узнать
 * географическое положение по IP-адресу.
 * Из документации следует, что для этого нужно отправить GET-запрос по адресу http://ipwhois.app/json/{IP},
 * где вместо {IP} следует подставить интересующий IP-адрес. Характеристики ответа настраиваются
 * с помощью параметров строки запроса. Например, параметр lang поможет настроить локализацию некоторых полей.
