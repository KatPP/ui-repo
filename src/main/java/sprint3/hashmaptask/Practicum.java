    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (command == 1) {
                ...
                System.out.println("Всего потрачено: " + expensesManager.getExpensesSum() + " руб.");
            } else if (command == 8) {
                System.out.println("Какую категорию вы хотите удалить?");
                String category = scanner.next();
                expensesManager.removeCategory(category);
            } else if (command == 9) {
                String maxCategory = expensesManager.getMaxCategoryName();
                if (!maxCategory.equals("")) {
                    System.out.println("В категории " + maxCategory + " вы потратили больше всего.");
                } else {
                    System.out.println("Пока нет ни одной категории с тратами.");
                }
            } else if (command == 0) {
